...
class DBResSpec extends FlatSpec with Matchers {
  ...
  "Method map of DBRes" should "be work correct" in {
    DBRes.update("DROP TABLE people", List.empty)
      .map(_ => DBRes.select("SELECT name FROM people", List.empty)(x => x.getString("count"))).execute(uri) shouldBe List()
  }

  "Method flatMap of DBRes" should "be work correct" in {
    DBRes.update("DROP TABLE people", List.empty)
      .flatMap(_ => DBRes.update("CREATE TABLE people(name VARCHAR(256), birthday DATE)", List.empty)
